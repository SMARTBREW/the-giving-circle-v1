...
function NavArrow({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-[#BDBDBD] bg-[#FFFFFF] transition-colors hover:border-[#A1A1AA] sm:h-14 sm:w-14 min-[90rem]:h-16 min-[90rem]:w-16"
    >
      <svg
        aria-hidden
        viewBox="0 0 14 24"
        fill="none"
        className="h-[1.25rem] w-[0.75rem] text-[var(--Main-CTA-button,#00A3BE)] min-[90rem]:h-[1.458rem] min-[90rem]:w-[0.859rem]"
      >
        <path
          d={
            direction === "prev"
    ...
    <section id="voices" className="w-full bg-[#FFFFFF]">
      <FadeInSection className="mx-auto flex w-full max-w-[90rem] flex-col items-center px-4 pt-10 pb-12 sm:px-8 sm:pt-14 sm:pb-16 md:px-10 md:pt-16 md:pb-16 lg:px-12 lg:pt-16 lg:pb-16 min-[90rem]:h-[60.4375rem] min-[90rem]:px-[6.25rem] min-[90rem]:pt-[5rem] min-[90rem]:pb-[5rem]">
        <SectionIntro
          ...
          title={title}
          subtitle={subtitle}
          titleClassName="text-[#1D2D23] min-[90rem]:h-16 min-[90rem]:w-[39.75rem] min-[90rem]:whitespace-nowrap"
          ...
          <div className="flex w-full items-center justify-center gap-3 sm:gap-4 min-[56.25rem]:gap-4 lg:gap-5 min-[90rem]:gap-6">
            ...
            >
              ...
              <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col px-5 py-6 sm:px-7 sm:py-8 md:px-8 min-[56.25rem]:px-6 min-[56.25rem]:py-6 lg:px-8 lg:py-8 min-[90rem]:px-8 min-[90rem]:pt-10 min-[90rem]:pb-0">
                ...
                <h3
                  className={`${SEGOE_UI_CLASS} shrink-0 text-[1.25rem] font-[700] leading-7 tracking-normal text-[var(--Main-headings,#000000)] sm:text-[1.5rem] sm:leading-8 lg:text-[1.625rem] lg:leading-8 min-[90rem]:w-full min-[90rem]:max-w-[33.5rem] min-[90rem]:text-[1.75rem] min-[90rem]:leading-9`}
                >
                  {active.title}
                </h3>

                <p
                  className={`${SEGOE_UI_CLASS} mt-3 shrink-0 text-[0.9375rem] font-[400] leading-6 tracking-normal text-[var(--Paragraph,#5F6D64)] sm:mt-4 sm:text-[1rem] sm:leading-7 lg:leading-8 min-[90rem]:mt-5 min-[90rem]:h-32 min-[90rem]:w-full min-[90rem]:max-w-[34rem] min-[90rem]:text-[1.0625rem] min-[90rem]:leading-8`}
                >
                  {active.body}
                </p>

                <div className="mt-8 flex min-h-0 w-full flex-1 flex-col sm:mt-10 min-[90rem]:mt-10">
                  <div className="w-full max-w-[32rem] shrink-0 border-t border-[#BDBDBD]" aria-hidden />
                  <div className="my-auto flex w-full max-w-[32rem] items-center gap-3 py-5 sm:gap-4 sm:py-6 min-[90rem]:py-0">
                    ...
                    <span className="min-w-0 text-left">
                      <span
                        className={`${SEGOE_UI_CLASS} block text-[1rem] font-[700] leading-5 tracking-normal text-[var(--Main-headings,#000000)] sm:text-[1.125rem] min-[90rem]:text-[1.5rem] min-[90rem]:leading-5`}
                      >
                        {active.name}
                      </span>
                      <span
                        className={`${SEGOE_UI_CLASS} mt-1 block text-[0.875rem] font-[400] leading-5 tracking-normal text-[#6F6C90] sm:text-[1rem] min-[90rem]:mt-2.5 min-[90rem]:text-[1.125rem] min-[90rem]:leading-5`}
                      >
                        {active.role}
                      </span>
                    </span>
                ...
                <button
                  ...
                  className={`h-5 w-5 rounded-full transition-colors ${
                    isActive
                      ...
                      : "border border-[#A1A1AA] bg-transparent"
                  }`}
                />
              );
            })}
