...
export default function CauseDetailHero({ cause }: { cause: LiveCause }) {
  return (
    <section
      className={`relative w-full overflow-x-hidden bg-[#eaf4f8] ${PAGE_HERO_BLEED}`}
    >
      {/* Soft brand-color ambient motion */}
      <div
        aria-hidden
        className="pointer-events-none absolute -top-24 left-[-6rem] h-64 w-64 rounded-full bg-[var(--Main-CTA-button,#02938c)]/15 blur-3xl motion-safe:animate-pulse"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute top-40 right-[-4rem] h-56 w-56 rounded-full bg-[var(--Brand-Green-Teal,#02938c)]/15 blur-3xl motion-safe:animate-pulse [animation-delay:1s]"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute bottom-10 left-[35%] h-44 w-44 rounded-full bg-[var(--Brand-Coral,#e62b4f)]/10 blur-3xl motion-safe:animate-pulse [animation-delay:2s]"
      />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[90rem] px-4 pt-6 pb-10 sm:px-8 sm:pt-8 sm:pb-12 md:px-10 md:pt-10 md:pb-14 lg:px-12 lg:pt-10 lg:pb-16 min-[90rem]:px-[6.25rem] min-[90rem]:pt-12 min-[90rem]:pb-20">
        <nav
          ...
          className={`${SEGOE_UI_CLASS} flex flex-wrap items-center gap-1.5 text-[0.8125rem] font-[500] text-[var(--Paragraph,#4a5558)] sm:text-[0.875rem]`}
        >
          <Link
            href="/"
            className="text-[var(--Main-CTA-button,#02938c)] transition-colors hover:text-[var(--Brand-Green-Teal,#02938c)]"
          >
            Home
          </Link>
        ...
        <FadeInSection className="mt-8 w-full sm:mt-10 lg:mt-12">
          ...
          <h1 className="mt-3 max-w-none font-['Georgia'] text-[1.75rem] font-[700] leading-9 tracking-normal text-[var(--Main-headings,#1c2426)] sm:mt-4 sm:text-[2.25rem] sm:leading-[2.75rem] md:text-[2.5rem] md:leading-[3rem] lg:text-[2.75rem] lg:leading-[3.25rem] min-[90rem]:text-[3rem] min-[90rem]:leading-[3.75rem]">
            {cause.title}
          </h1>

          <p
            className={`${SEGOE_UI_CLASS} mt-4 w-full max-w-none text-[0.9375rem] leading-6 font-[400] text-[#4a5558] sm:mt-5 sm:text-[1.0625rem] sm:leading-7 md:text-[1.125rem] md:leading-8 lg:text-[1.25rem] lg:leading-9`}
          >
            {cause.summary}
          </p>
          ...
          <p
            className={`${SEGOE_UI_CLASS} mt-5 text-[0.875rem] font-[400] text-[var(--Paragraph,#4a5558)] sm:mt-6 sm:text-[0.9375rem]`}
          >
            By{" "}
